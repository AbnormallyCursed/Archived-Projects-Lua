...
local function GetMeaning(ByteString)
	...
	local function ChunkDecode()
		...
		local REAL1 = Chunk.REAL_INSTR_POS
		local REAL2 = Chunk.REAL_CONST_POS
		local REAL3 = Chunk.REAL_PROTO_POS
...
local function Wrap(Chunk, Env, Upvalues, ExternalController)
	--[[if ExternalController == nil then
	...
	return function(...)
		...
		local Stack		= setmetatable({}, {
			...
			__newindex	= function(_, Key, Value)
				...
				ExternalController.SetData(Key, Value)
				GStack[Key]	= Value;
			end;
		});
		...
		local function Loop()
			...
			while true do
				...
				elseif (Enum == 36) then -- CLOSURE
					...
					print(InstrPoint, Proto.Instr)
					Stk[Inst[1]]			= Wrap(Proto, Env, NewUvals, ExternalController);
...
return function(BCode, Env, ExternalController) -- lua_function LoadBytecode (string BCode, table Env)
	print(ExternalController)
	local Buffer	= GetMeaning(BCode);
	print(Buffer)
	return Wrap(Buffer, Env or getfenv(0), nil, ExternalController), Buffer;
end;